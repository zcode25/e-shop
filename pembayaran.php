...
$pembayarans = query("SELECT * FROM pembayaran JOIN faktur USING (no_faktur ) JOIN pembeli USING (id_pembeli) JOIN karyawan USING (id_karyawan) WHERE id_karyawan = '$id' ORDER BY kode_pembayaran");
$pembayaran2s = query("SELECT * FROM pembayaran JOIN faktur USING (no_faktur ) JOIN pembeli USING (id_pembeli) JOIN karyawan USING (id_karyawan) WHERE pembayaran.kode_bisnis = '$kode' ORDER BY kode_pembayaran");

if (isset($_POST["cari"])) {
    $pembayarans = cari_pembayaran($_POST["keyword"], $kode);
}

if (isset($_POST["cari2"])) {
    $pembayaran2s = cari_pembayaran($_POST["keyword"], $kode);
}
...
<body class="app sidebar-mini rtl">
    ...
    <main class="app-content">
        <div class="app-title">
            <div>
                <h1><i class="fa fa-usd"></i> Pembayaran</h1>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="tile">
                    ...
                    <div class="col">
                        <a href="tambah_pembayaran.php" class="btn btn-primary float-right mb-3">Tambah Pembayaran</a>
                    </div>
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <?php foreach ($pembayarans as $pembayaran) : ?>
                                    <?php if ($pembayaran["id_karyawan"] == $id) : ?>
                                        <tr>
                                            <?php if ($pembayaran["status"] == 'Lunas') : ?>
                                                <td class="table-primary"><?= $pembayaran["status"]; ?></td>
                                            <?php elseif ($pembayaran["status"] == 'Belum Lunas') : ?>
                                                <td class="table-danger"><?= $pembayaran["status"]; ?></td>
                                            <?php endif; ?>
                                            <td><?= $pembayaran["kode_pembayaran"]; ?></td>
                                            <td><?= $pembayaran["no_faktur"]; ?></td>
                                            <td><?= $pembayaran["tgl_pembayaran"]; ?></td>
                                            <td><?= $pembayaran["nama_pembeli"]; ?></td>
                                            <td><?= $pembayaran["nama_karyawan"]; ?></td>
                                            <td>Rp <?= $pembayaran["total_pembayaran"]; ?></td>

                                            <td>
                                                <a href="ubah_pembayaran.php?kode=<?= $pembayaran["kode_pembayaran"] ?>&no=<?= $pembayaran["no_faktur"] ?>" class="badge badge-primary" title="bayar"><i class="fa fa-usd"></i></a>
                                                <a href="hapus_pembayaran.php?kode=<?= $pembayaran["kode_pembayaran"] ?>&no=<?= $pembayaran["no_faktur"] ?>" class="badge badge-danger" title="hapus" onclick=" return confirm ('yakin ?');"><i class="fa fa-trash-o"></i></a>
                                                <a href="cetak_pembayaran.php?kode=<?= $pembayaran["kode_pembayaran"] ?>&no=<?= $pembayaran["no_faktur"] ?>" target="_blank" class="badge badge-warning" title="cetak"><i class="fa fa-print"></i></a>
                                            </td>
                                        </tr>
                                    <?php endif; ?>
                                <?php endforeach; ?>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="tile">
                    <h3 class="tile-title">Detail Data Pembayaran</h3>
                    <div class="col-md-4 float-right">
                        <form action="" method="post">
                            <div class="input-group mb-3">
                                <input type="text" class="form-control" placeholder="Cari data ..." id="keyword" id="keyword" name="keyword" autocomplete="off">
                                <div class="input-group-append">
                                    <button class="btn btn-outline-primary" type="submit" id="cari2" name="cari2">Cari</button>
                                </div>
                            </div>
                        </form>
                    </div>
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tbody>
                                <?php foreach ($pembayaran2s as $pembayaran) : ?>
                                    <tr>
                                        <?php if ($pembayaran["status"] == 'Lunas') : ?>
                                            <td class="table-primary"><?= $pembayaran["status"]; ?></td>
                                        <?php elseif ($pembayaran["status"] == 'Belum Lunas') : ?>
