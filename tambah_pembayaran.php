...
$pembayaran = query("SELECT max(kode_pembayaran) as kode_terbesar FROM pembayaran")[0];
$kode_pembayaran = $pembayaran["kode_terbesar"];
$urutan = (int) substr($kode_pembayaran, 3, 4);

$urutan++;
$huruf = "PMB";
$kode_pembayaran = $huruf . sprintf("%04s", $urutan);

$id = $_SESSION["id"];
$fakturs = query("SELECT * FROM faktur WHERE id_karyawan = '$id'");


if (isset($_POST["submit"])) {

  $no = $_POST["faktur"];
  $cek_faktur = query("SELECT no_faktur FROM pembayaran WHERE no_faktur = '$no'");
  ...
  if ($cek_faktur == null) {
    ...
    echo "
        ...
        <script>
          alert('Data pembayaran sudah ada');
          document.location.href = 'pembayaran.php';
        </script>
...
<body class="app sidebar-mini rtl">
  ...
  <main class="app-content">
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="tile">
          <h3 class="tile-title">Tambah Data Pembayaran</h3>
          <div class="tile-body">
            <form action="" method="post">
              <!-- <div class="form-group"> -->
              <!-- <label class="control-label" for="kode_pembayaran">Kode Pembayaran</label> -->
              <input class="form-control" type="hidden" placeholder="Masukan kode pembayaran" id="kode_pembayaran" name="kode_pembayaran" value="<?= $kode_pembayaran; ?>" required>
              <input class="form-control" type="hidden" id="kode_bisnis" name="kode_bisnis" value="<?= $kode; ?>" required>
              <!-- </div> -->
              <div class="form-group">
                <label for="faktur">Faktur</label>
                <select class="form-control" id="faktur" name="faktur">
              ...
              <div class="form-group">
                ...
                <input class="form-control" type="date" placeholder="Masukan tanggal pembayaran" id="tgl_pembayaran" name="tgl_pembayaran" required>
              </div>
              <div class="form-group">
                <!-- <label class="control-label" for="total_pembayaran">Total Pembayaran</label> -->
                <input class="form-control" type="hidden" placeholder="Masukan total pembayaran" id="total_pembayaran" name="total_pembayaran" value="0" required>
              </div>
          </div>
