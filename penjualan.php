...
$penjualans = query("SELECT * FROM faktur JOIN pembeli USING (id_pembeli) JOIN karyawan USING (id_karyawan) WHERE id_karyawan = '$id' ORDER BY no_faktur");
$penjualan2s = query("SELECT * FROM faktur JOIN pembeli USING (id_pembeli) JOIN karyawan USING (id_karyawan) WHERE faktur.kode_bisnis = '$kode' ORDER BY no_faktur");


if (isset($_POST["cari"])) {
  $penjualans = cari_penjualan($_POST["keyword"], $kode);
}


if (isset($_POST["cari2"])) {
  $penjualan2s = cari_penjualan($_POST["keyword"], $kode);
}
...
<body class="app sidebar-mini rtl">
  ...
  <main class="app-content">
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="tile">
          ...
          <div class="col">
            <a href="tambah_penjualan.php" class="btn btn-primary float-right mb-3">Tambah Penjualan</a>
          </div>
            ...
            <table class="table">
              ...
              <tbody>
                <?php foreach ($penjualans as $penjualan) : ?>
                  <?php if ($penjualan["id_karyawan"] == $id) : ?>
                    <tr>
                      ...
                      <td>
                        <a href="transaksi_penjualan.php?no=<?= $penjualan["no_faktur"] ?>" class="badge badge-info" title="transaksi"><i class="fa fa-shopping-cart"></i></a>
                        <a href="ubah_penjualan.php?no=<?= $penjualan["no_faktur"] ?>" class="badge badge-primary" title="ubah"><i class="fa fa-pencil"></i></a>
                        <a href="hapus_penjualan.php?no=<?= $penjualan["no_faktur"] ?>" class="badge badge-danger" title="hapus" onclick=" return confirm ('yakin ?');"><i class="fa fa-trash-o"></i></a>
                        <a href="cetak_penjualan.php?no=<?= $penjualan["no_faktur"] ?>" class="badge badge-warning" title="cetak" target="_blank"><i class="fa fa-print"></i></a>
                      </td>
                    </tr>
                  <?php endif; ?>
                <?php endforeach; ?>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="tile">
          <h3 class="tile-title">Detail Data Penjualan</h3>
          <div class="col-md-4 float-right">
            <form action="" method="post">
              <div class="input-group mb-3">
                <input type="text" class="form-control" placeholder="Cari data ..." id="keyword" id="keyword" name="keyword" autocomplete="off">
          ...
          <div class="table-responsive">
            <table class="table">
              ...
              <tbody>
                <?php foreach ($penjualan2s as $penjualan) : ?>
                  <tr>
                    <td><?= $penjualan["no_faktur"]; ?></td>
                    <td><?= $penjualan["tgl_pemesanan"]; ?></td>
                    <td><?= $penjualan["tgl_jatohtempo"]; ?></td>
